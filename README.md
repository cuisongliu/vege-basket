...
- **Veges AI**：个人加密对话历史可跨刷新恢复；单一聊天输入面支持文本附件和不可变的普通、`@项目`、对话分析上下文，统一记录普通回复、项目日/周总结、Markdown 待办提案和对话分析结果。
...
- AI 对话由服务端保存 canonical turns，浏览器不能提交或伪造助手历史；普通对话不会隐式读取整个工作区。对话标题、用户/助手正文和附件名称/正文均加密，历史接口只返回附件元数据；删除后的会话 UUID 会保留墓碑，晚到请求不能把历史重新创建出来。
...
当前 Node 测试覆盖通知策略、API 错误信息脱敏、OSS Endpoint 规范化、AI Provider 安全、AI 会话领域与客户端状态、AI 周期与待办提案解析、AI 限流和日报调度。涉及待办并发、AI turn 租约、数据库结构、项目失权或历史数据加密的变更，需要在明确授权的隔离数据库中补充集成验证。
