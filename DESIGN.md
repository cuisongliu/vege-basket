...
The current-day/week progress example is an explicit workspace review, not generic chat. Without `@项目`, it reads authorized cross-project facts from the server; with `@项目`, the same natural wording stays scoped to the selected project. Workspace reviews show concise preparation and generation phases before replacing them with the canonical final response, without exposing partial project-derived text.

Before a canonical turn is created, the composer may briefly show `正在理解请求` while the server resolves semantic intent. Keep the source draft visible, disable edits that would change its content or context, and turn the send control into a stop action that cancels this classification. The same cancellation applies when starting a new conversation, changing project context, deleting the current conversation, or leaving the surface.
